...
(function(ROOT, struct, undefined) {
  ...
  struct.prototype = {
        ...
        case 3:
          if (canDrag) {
            ...
            if (ev.length && (ev.eventCode == 1 || this.drag)) {
              ...
              each("rect drag time startPos _offset".split(" "), function(
                prop
              ) {
                delete self[prop];
              });
    ...
    getSum: function(from, to) {
      var sum = 0;
      while (from < to) {
        sum += Math.ceil(this.getOuterSize(this.pages[from++], true));
      }
      return sum;
    },
    ...
    getOuterSize: function(elem, withMargin) {
      ...
      let result = (
        // Math.ceil(elem[this.direction ? "offsetHeight" : "offsetWidth"]) +
        Math.ceil(computedStyle.width.replace('px', '')) +
        Math.ceil((withMargin ? this.getMarginSize(elem) : 0))
      );
      // console.log(result, elem[this.direction ? "offsetHeight" : "offsetWidth"], Math.ceil(elem[this.direction ? "offsetHeight" : "offsetWidth"]), Math.ceil((withMargin ? this.getMarginSize(elem) : 0)))
      return result
    },
